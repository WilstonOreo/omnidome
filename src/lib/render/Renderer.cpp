...
namespace omni {
  namespace render {
    Renderer::Renderer(Session& _session) :
      session_(_session)
    {}
...
// Model view operation
                     [&](QOpenGLFunctions& _)
      {
        // Draw canvas with
        _sessionViz.update();
        _sessionViz.drawCanvas(session_.exportSettings().mappingOutputMode());
      }
                     );
...
// Model view operation
                          ...
                          int _height) const
    {
      ...
      if (_settings.mappingOutputMode() == mapping::OutputMode::MAPPED_INPUT)
      {
        _image = QImage(_w, _overallHeight, QImage::Format_RGB32);
        bufferToImage(_buffer, _image,
      ...
      else
      {
        ...
        if (_settings.mappingOutputMode() == mapping::OutputMode::TEXCOORDS) {
        ...
        if (_settings.mappingOutputMode() == mapping::OutputMode::UVW) {
    ...
    void Renderer::getAlphaMask(RenderBuffer const& _buffer,
                                QImage& _image,
