    ...
    namespace visual {
        ...
        void Light::setup(GLuint _index)
        {
            ...
            visual::with_current_context([&](QOpenGLFunctions& _)
            {
                ...
                glLightf(_index, GL_CONSTANT_ATTENUATION,  Kc);
                glLightf(_index, GL_LINEAR_ATTENUATION,    Kl);
                glLightf(_index, GL_QUADRATIC_ATTENUATION, Kq);

                // -------------------------------------------
                // Lighting parameters:

                glLightfv(_index, GL_POSITION, eye4().data());
                glLightfv(_index, GL_AMBIENT,  colorParam(ambient()).data());
                glLightfv(_index, GL_DIFFUSE,  colorParam(diffuse()).data());
                glLightfv(_index, GL_SPECULAR, colorParam(specular()).data());
