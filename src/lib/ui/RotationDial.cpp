...
#include <omni/ui/RotationDial.h>
...
namespace omni {
    namespace ui {
        ...
        RotationDial::RotationDial(double   _x,
                                   double   _y,
                                   double   _z,
                                   QWidget *_parent) :
            QWidget(_parent),
        ...
        void RotationDial::mousePressEvent(QMouseEvent *e)
        {
            ...
            int _circle =  QVector2D(rect().center() - e->pos()).length() /
                          radius() * 5;
        ...
        {
            if (isMoving()) valueLabel_->setText(formatDegrees(_val));
        ...
        double RotationDial::getValue(QPoint const& _point)
        {
            ...
            return snap() ? int(_angle / singleStep() + 0.5) *
                   singleStep()  : _angle;
        }
        ...
        void RotationDial::paintPies(QPainter& _p)
        {
            ...
            paintPie(_p, _r * 0.9, _r * 1.0,
                     x(), moveState_ == MOVE_X ? _hColor : xColor());
            paintPie(_p, _r * 0.7, _r * 0.8,
                     y(), moveState_ == MOVE_Y ? _hColor : yColor());
            paintPie(_p, _r * 0.5, _r * 0.6,
                     z(), moveState_ == MOVE_Z ? _hColor : zColor());
        }

        void RotationDial::paintPie(QPainter& _p,
                                    double    _innerRadius,
                                    double    _outerRadius,
                                    double    _value,
                                    QColor    _color)
        {
            ...
            if (showTicks())
            {
                for_each_step(minimum(), maximum(),
                              [&](int _step, double i, bool _isPage)
                {
        ...
        void RotationDial::paintEvent(QPaintEvent *e)
        {
            ...
            if (moveState_ !=
                MOVE_X) _p.drawText(QRect(rect().left(),
                                          _center.y() - radius() * 1.08,
                                          width(), _r * 0.5), Qt::AlignCenter,
                                    "X");

            if (moveState_ !=
                MOVE_Y) _p.drawText(QRect(rect().left(),
                                          _center.y() - radius() * 0.88,
                                          width(), _r * 0.5), Qt::AlignCenter,
                                    "Y");

            if (moveState_ !=
                MOVE_Z) _p.drawText(QRect(rect().left(),
                                          _center.y() - radius() * 0.68,
                                          width(), _r * 0.5), Qt::AlignCenter,
                                    "Z");
            ...
            if (moveState_ !=
                MOVE_X) _p.drawText(QRect(rect().left(),
                                          _center.y() + radius() * 0.60,
                                          width(), _r * 0.5), Qt::AlignCenter,
                                    formatDegrees(x_));

            if (moveState_ !=
                MOVE_Y) _p.drawText(QRect(rect().left(),
                                          _center.y() + radius() * 0.40,
                                          width(), _r * 0.5), Qt::AlignCenter,
                                    formatDegrees(y_));

            if (moveState_ !=
                MOVE_Z) _p.drawText(QRect(rect().left(),
                                          _center.y() + radius() * 0.20,
                                          width(), _r * 0.5), Qt::AlignCenter,
                                    formatDegrees(z_));
        ...
        void RotationDial::paintTick(QPainter& _p,
                                     double    _inner,
                                     double    _outer,
                                     double    _value)
        {
