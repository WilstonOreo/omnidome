...
void main()
{
 ...
 // uv.y = 1.0 - uv.y;
    ...
    vec2 s = _normal_map_imgSize;
    vec2 uv = vv_FragNormCoord + vec2(0.0,1.0/_normal_map_imgSize.y);

    vec2 normal_upper = vec2(uv.x*s.x,(uv.y*s.y + 2.0*s.y)/3.0);
    vec2 normal_lower = vec2(uv.x*s.x,(uv.y*s.y + s.y)/3.0);
    vec2 blendmask = vec2(uv.x*s.x,uv.y*s.y/3.0);
    

    vec3 n = texture2DRect(normal_map,normal_upper).xyz*2.0 - 1.0 + texture2DRect(normal_map,normal_lower).xyz*2.0/255.0 - 1.0 / 255.0;
  ...
  if (map_mirror_vertical)    
  {
    ...
    float alpha =  texture2DRect(normal_map,blendmask).r;
