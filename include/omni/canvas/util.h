  ...
  namespace canvas
  {
    namespace util
    {
      ...
      static void generateCone(size_t _slices,
                          ...
                          float _radius, VERTICES& _vertices, INDICES& indices)
      {
        ...
        for_each_circle_point(_slices,_radius,[&](size_t i, const QPointF& _p2)
        {
          ...
          _vertices.emplace_back(_p,_normal,QPointF(float(i)/_slices,acos(_normal.z()) / M_PI));
          if (_zTop > _zBottom)
          {
                                 ...
                                 VERTICES& _vertices, INDICES& indices)
      {
        ...
        for (size_t i = 0; i <= _slices; ++i)
        {
          ...
          _vertices.emplace_back(_top,_normalTop,QPointF(float(i)/_slices,acos(_normalTop.z()) / M_PI));
          _vertices.emplace_back(_bottom,_normalBottom,QPointF(float(i)/_slices,acos(_normalBottom.z()) / M_PI));
        }
      ...
      /// Top cone
      generateCone(_slices,1.0,stackPos(1),stackRadius(1),_vertices,indices);
      /// Bottom cone
      generateCone(_slices,-1.0,stackPos(_stacks - 1),stackRadius(1),_vertices,indices);
